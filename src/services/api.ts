...
// ========================
// FRONTEND API FUNCTIONS - Connected to Next.js API Routes
// ========================

// Main content generation - connects to /api/generate
export const generateContent = async (
  request: GenerationRequest
): Promise<GeneratedItem> => {
  try {
...
// Generate post - connects to /api/generate-post
export const generatePost = async (
  prompt: string,
  options?: {
    length?: "short" | "medium" | "long" | "extended";
  ...
  try {
    const response = await apiClient.post("/generate-post", {
      prompt,
      options,
    });
...
// Generate image - connects to /api/generate-image
export const generateImage = async (
  prompt: string,
  size: ImageSize = "medium"
): Promise<string> => {
  try {
    ...
    console.log("Image generation successful:", {
      url: response.data.imageUrl.substring(0, 100) + "...",
      size,
    });
...
// Generate template - connects to /api/generate-template
export const generateTemplate = async (
  prompt: string
): Promise<{ content: string; imageUrl?: string }> => {
  try {
    const response = await apiClient.post("/generate-template", {
      prompt,
    });
...
// Generate batch content - connects to /api/generate-batch
export const generateBatch = async (
  prompts: string[],
  type: ContentType,
  size?: ImageSize
  ...
  try {
    console.log(`Calling /api/generate-batch for ${prompts.length} items`);
    ...
    console.log(
      `Batch generation completed: ${response.data.items.length}/${prompts.length} successful`
    );
...
// Generate image variations - connects to /api/generate-variations
export const generateImageVariations = async (
  basePrompt: string,
  variations: string[],
  size: ImageSize = "medium"
  ...
  try {
    console.log(
      `Calling /api/generate-variations for ${variations.length} styles`
    );

    const response = await apiClient.post("/generate-variations", {
      basePrompt,
      variations,
      size,
    });
    ...
    console.log(
      `Image variations completed: ${response.data.variations.length}/${variations.length} successful`
    );
...
// Generate platform-specific post - connects to /api/generate-platform-post
export const generatePlatformSpecificPost = async (
  prompt: string,
  platform:
    | "twitter"
  ...
  try {
    console.log(`Calling /api/generate-platform-post for ${platform}`);

    const response = await apiClient.post("/generate-platform-post", {
      prompt,
      platform,
      includeImage,
    });
    ...
    console.log(`${platform} post generated:`, {
      characterCount: response.data.metadata.characterCount,
      withinLimit:
        response.data.metadata.characterCount <=
        response.data.platformGuidelines.characterLimit,
...
// Generate complete social media package - connects to /api/generate-social-package
export const generateSocialMediaPackage = async (
  prompt: string,
  options?: {
    postLength?: "short" | "medium" | "long" | "extended";
  ...
  try {
    console.log("Calling /api/generate-social-package...");

    const response = await apiClient.post("/generate-social-package", {
      prompt,
      options,
    });

    console.log("Social media package completed:", {
      postWordCount: response.data.post.metadata.wordCount,
      imageCount: response.data.images.length,
      hashtags: response.data.post.metadata.hashtags.length,
    });
...
// Health check - connects to /api/health
export const healthCheck = async (): Promise<{
  llm: boolean;
  pollinations: boolean;
  overall: boolean;
...
// API status - enhanced health check
export const getApiStatus = async () => {
  const health = await healthCheck();
...
// Test endpoint - connects to /api/test
export const testApiConfiguration = async (): Promise<{
  success: boolean;
  message: string;
  details: any;
